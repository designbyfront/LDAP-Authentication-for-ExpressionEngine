...
// If you are looking here to edit the settings, you can always just change them in Extensions settings page :)
	...
	// PHP4 Constructor
	function Ldapplugin_ext($settings = '')
	{
...
// ----------------------
	...
	{
		$this->debug_print('login_authenticate_start');
...
// ----------------------
	...
	function login_authenticate_start()
	{
		...
		{
			$this->debug_print('creating connection');
			...
			$this->debug_print('authenticating');
...
// ----------------------
	...
	{
		$this->debug_print('Syncing user details ...');
		
		// Sync EE password to match LDAP (if account exists)
		$encrypted_password = hash('sha1',stripslashes($user_info['password'])); // $this->EE->functions->hash(stripslashes($user_info['password']));
		$this->debug_print('encrypted pw: '.$encrypted_password);
...
// ----------------------
	...
	{
		$this->debug_print('Searching for attribute ('.$ldap_username_attribute.'='.$username.') in base '.$ldap_search_base.' ...');
		...
		{
			$this->debug_print('User successfully authenticated against LDAP');
		}
